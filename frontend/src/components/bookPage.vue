<template>
  <div class="page-box-book">
    <div class="app-header">
      <div class="home-btn">
        <ElBreadcrumb separator="/">
          <ElBreadcrumbItem :to="{ path: '/home/booklist' }" style="font-size: 20px"
            ><p @click="handleSaveProgress">< My Shelf</p></ElBreadcrumbItem
          >
          <ElBreadcrumbItem v-if="isLoading"
            ><p class="book-name">Loading...</p></ElBreadcrumbItem
          >
          <ElBreadcrumbItem v-else
            ><p class="book-name">{{ route.query.name }}</p></ElBreadcrumbItem
          >
        </ElBreadcrumb>
      </div>
      <div class="tool-bar">
    ...
    <div class="app-content">
      ...
      />
      <ElIcon
        style="position: absolute; top: 50%; left: -10%; color: white; cursor: pointer"
        size="80"
      ...
      /></ElIcon>
      <ElIcon
        style="position: absolute; top: 50%; right: -10%; color: white; cursor: pointer"
        size="80"
    ...
    <el-progress
      ...
      :show-text="true"
    />
    <p style="position: fixed; top: 110px; right: 10px; color: white">
      Reading Percentage
    </p>
    <p style="position: fixed; top: 170px; right: 50px; color: white">
      {{ pagePercentage }}%
    </p>
  </div>
</template>
...
const saveReadProgress = async () => {
  ...
  await saveProgress({
    ...
    loading2.close();
    notify({
      title: "Reading Progress Saved for " + param.bookName,
      duration: 3000,
...
onMounted(() => {
  page.value =
    parseInt(route.query.currentPage, 10) % 2 == 0
      ? parseInt(route.query.currentPage, 10) - 1
      : parseInt(route.query.currentPage, 10);
  secondPage.value = page.value + 1;
  param.bookName = route.query.name;
